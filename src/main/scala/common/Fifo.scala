...
import harness.Formal
import harness.Utils.implies
import harness.Utils.anyconst
import harness.Check
import harness.Cover
import harness.Bmc
import harness.Prove
import harness.DecoupledProperties
...
class Fifo[T <: Data](gen: T, bufferSize: Int, emitFormal: Boolean = true) extends Module {
  ...
  if (emitFormal) {
    ...
    DecoupledProperties.emitProducer(io.deq)
    DecoupledProperties.emitConsumer(io.enq)
  }
}
