...
/** Lightweight helpers for testing cherrytrunk slave devices -- the bus analog of
  * `cpu.isa.CpuTestBase`. No golden model or compliance battery: just spin up the DUT with a
  * [[BusMaster]] attached and write plain `poke`/`read`/`write`/`expect` assertions per test.
  */
trait CherrytrunkSlaveTestBase extends Matchers with ChiselSim { self: TestSuite =>

  /** Simulate `dut`, attach a [[BusMaster]] to the bus port returned by `bus`, settle reset, then
    * hand the DUT and master to `body`.
    */
