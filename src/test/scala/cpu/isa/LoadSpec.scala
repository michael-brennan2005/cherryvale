...
/** Loads: lw, lh, lhu, lb, lbu. RISC-V memory is little-endian, so the word
  * 0xAABBCCDD stored at 0x50 lays out as byte[0x50]=0xDD .. byte[0x53]=0xAA.
  */
class LoadSpec extends AnyFlatSpec with CpuTestBase {
  ...
  it should "lw with a negative offset" in {
    run("""addi x2, x0, 0x54
           lw x1, -4(x2)
        """, data = Seq(0x50 -> BigInt("12345678", 16))) { dut =>
