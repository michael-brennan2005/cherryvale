...
/** Branches: beq, bne, blt, bge, bltu, bgeu.
  *
  * Pattern: set up operands, set a marker x10 = 5, then branch over a "poison"
  * instruction that would set x10 = 9. Taken => x10 stays 5; not-taken => x10 == 9.
  * Operand setup is separated from the branch by the marker instruction so the branch
  * reads settled registers (isolating branch semantics from tight forwarding).
  */
