  ...
  it should "addi with immediate 0 acts as a move" in {
    run(
      """lw x1, 0x50(x0)
           addi x2, x1, 0
        """,
      data = Seq(0x50 -> BigInt("DEADBEEF", 16))
    ) { dut =>
  ...
  it should "xori with -1 is a bitwise NOT" in {
    run(
      """lw x1, 0x50(x0)
           xori x2, x1, -1
        """,
      data = Seq(0x50 -> BigInt("0F0F0F0F", 16))
    ) { dut =>
  ...
  it should "ori sets bits (immediate sign-extended)" in {
    run(
      """lw x1, 0x50(x0)
           ori x2, x1, 0xF
        """,
      data = Seq(0x50 -> BigInt("F0F0F0F0", 16))
    ) { dut =>
  ...
  it should "srai on a positive value matches srli" in {
    run(
      """lw x1, 0x50(x0)
           srai x2, x1, 4
        """,
      data = Seq(0x50 -> BigInt("70000000", 16))
    ) { dut =>
